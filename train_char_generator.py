...
def create_model(modelname,vocab_size,num_units = 150,num_lstm_layers=1,optimizer=RMSprop(learning_rate=10e-3)
...
def create_callbacks(early_stop = True,es_patience=10,es_monitor= 'loss',restore_best_weights=True,checkpoint=False
                     ,checkpoint_filepath="tmp/checkpoints_chars_text_generator"):
    callback_list =list()
    if early_stop:
...
def pipeline(datapath,modelname,vocabpath,wp,game=None):
    ...
    model,modelname = create_model(modelname,len(vocab),num_units = 100,num_lstm_layers=2)
...
#python train_char_generator.py data/with_punct models/char_tg_all models/vocab_all_wp.txt True